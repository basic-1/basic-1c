...
- `PEEK(<numeric>)` - reads one byte value from the specified memory address  
...
If the `DIM` keyword is omitted all variables declared on a line beginning with the `CONST` keyword are constants, e.g. the next statement declares three constant variables:  
`CONST C1 = 10, C2 = 20, CARR = (1, 2, 3, 4, 5)`  
